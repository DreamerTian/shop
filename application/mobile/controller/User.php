...
//        }
    ...
    public function user_verify()
    {
        ...
        if(IS_POST){
            ...
            I('post.idcard') ? $post['idcard'] = I('post.idcard') : false; //头像地址
            I('post.cardtype') ? $post['cardtype'] = I('post.cardtype') : false;  // 性别
            if(!$userLogic->update_info($this->user_id,$post))
                $this->ajaxReturn( ['status' => false, 'msg' => '验证失败！', 'result' => '']);
            setcookie('uname',urlencode($post['nickname']),null,'/');
            $this->ajaxReturn( ['status' => true, 'msg' => '验证成功！', 'result' => '']);
            exit;
        }
     ...
     *  出入金记录
     * */
    public function out_in_list()
    {
        $where['user_id'] = $this->user_id;
        $count = M('cash_access')->where($where)->count();

        $page = new Page($count, 15);
        $list = M('cash_access')->where($where)->order("id desc")->limit("{$page->firstRow},{$page->listRows}")->select();

        $this->assign('page', $page->show());// 赋值分页输出
        $this->assign('list', $list); // 下线
        if (I('is_ajax')) {
            return $this->fetch('ajax_out_in_list');
        }
        return $this->fetch();
    }
}
